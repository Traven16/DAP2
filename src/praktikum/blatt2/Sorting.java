 ...
 * @version 0.0.0.4
 * @time 21.04.2015 10:53
 */

public class Sorting {
	
	/* Attributes */
	private static final byte INSERTIONSORT = 0, MERGESORT = 1;
	private static final Random random = new Random();
	private static final String infoMsg = "\nINFO:\nAufruf lediglich mit 1, 2 oder 3 Parametern gestattet:\n"
										+ "\t1. Parameter: Groesse des zu initialisierenden Feldes (positiver int Wert!)\n"
										+ "\t2. Parameter: Fuellart(\"auf\", \"ab\", \"rand\")\n"
										+ "\t3. Parameter: Zu verwendender Sortieralagorithmus(\"insertion\", \"merge\", \"all\")\n"
	...
	public static void main(String[] args) {
		...
		if(args.length > 2)
			if(args[2].equals("insertion") || args[2].equals("merge") || args[2].equals("all"))
	 ...
	 */
	private static void sortArrayWith(int[] array, final byte algorithmDeclaration) {
		System.out.print("\n");
	...
	private static void mergeSort(int[] array, int[] temp, final int left, final int right) {
	...
	private static void mergeSortMerge(int[] array, int[] temp, final int left, final int mid, final int right) {
	 ...
	 */
	public static void initRandom(int[] array, final int minValue, final int maxValue) {
	...
	/**
	 * Returns a random int value between min(inclusive) and max(icklusive).
